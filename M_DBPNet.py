...
class M_DBPNet(nn.Module):
    def __init__(self, in_channels):
        super(M_DBPNet, self).__init__()
        self.fre = Frequency_Branch()
        self.tem = Temporal_Branch(in_channels)
...
def train_model(settings, args, train_loader, valid_loader, test_loader, subject):
    ...
    def evaluate(model, criterion, test=False):
        ...
        with torch.no_grad():
            for i_batch, batch_data in enumerate(loader):
                ...
                total_loss += criterion(preds, test_label).item() * args.batch_size
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if val_loss < best_valid:
            ...
            best_epoch = epoch
            print(f"Saved model at pre_trained_models/{save_load_name(args, name=args.name)}_{args.window_length}.pt!")
...
def main(name="s7", data_document_path="dataset/AVED/prepro_ao_wav", length = 2):
    # seed_torch()
    args = DotMap()
    args.name = name
    ...
    subpath = args.data_document_path + '/'  + str(name) + '_prepro_av.mat'
    #subpath = args.data_document_path + '/'  + str(name) + '_mbssf_wav.mat'
    print(args.name)
    print(subpath)

    eeg_data, event_data = get_data_from_mat(subpath)
    data = np.vstack(eeg_data)
    label = np.vstack(event_data)


    # split window、testset
    train_eeg, test_eeg, train_label, test_label = sliding_window(eeg_data, label, args, args.csp_comp)
    eeg_data = data.reshape(args.trail_number, args.cell_number, args.eeg_channel)
    ...
    seq_train_data, seq_valid_data, fre_train_data, fre_valid_data, train_label, valid_label = train_test_split(
        seq_train_data, fre_train_data, train_label, test_size=0.1, random_state=42)
    ...
    print(seq_train_data.shape)
    print(seq_valid_data.shape)
...
if __name__ == "__main__":
    code_path = 'M-DBPNet/'
    path = "/dataset/AVED/prepro_av_wav"
    #lengths = [5]
    # lengths = [0.1, 1, 2]
    lengths = [1]
    names = ['s' + str(i+1) for i in range(0, 50)]
    for length in lengths:
        accs = []
        for name in names:
            loss, acc = main(name, path, length)
            accs.append(acc)
        avg_acc = sum(accs) / len(accs)
        df = pd.DataFrame({'subject': names + ['average'], 'acc': accs + [avg_acc]})
        csv_path = code_path + 'accs_' + str(length) + 's.csv'
